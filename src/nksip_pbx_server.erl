...
sip_get_user_pass(_User, <<?DOMAIN>>, _Req, _Call) ->
    <<"1234">>;
...
%%   ?DOMAIN. If `{{digest, <<?DOMAIN>>}, true}' is present, the user has 
%%   provided a valid password and it is authorized. 
%%   If `{{digest, <<?DOMAIN>>}, false}' is present, we have presented 
...
sip_authorize(Auth, Req, _Call) ->
    ...
    case lists:member(dialog, Auth) orelse lists:member(register, Auth) of
        true -> 
            ok;
        false ->
            case nklib_util:get_value({digest, <<?DOMAIN>>}, Auth) of
                true -> 
...
sip_route(Scheme, User, <<?DOMAIN>>, Req, _Call) ->
    case nksip_request:method(Req) of
        {ok, 'SUBSCRIBE'} ->
            {reply, forbidden};
        _ ->
            UriList = nksip_registrar:find(?MODULE, Scheme, User, <<?DOMAIN>>),
            {proxy, UriList, [record_route]}
    end;
